...
uint32 soem_check_network_configuration(const std::map<int, std::string> list_of_slaves)
{
  printf("[%sSTART%s] %s[ Names ]%s %sCheck Coherence between yaml and network ",
         BOLDMAGENTA(),
         RESET(),
         BOLDBLUE(),
         RESET(),
         BOLDYELLOW());
  printf("[-----] SOEM got %d slaves, while %zu were expected. ", ec_slavecount, list_of_slaves.size());
  /* Do we got expected number of slaves from config */
  if ((std::size_t)ec_slavecount < list_of_slaves.size())
  {
    printf(
        "[-----] SOEM got %d slaves, while %zu were expected. Abort. ", ec_slavecount, list_of_slaves.size());
    return 0;
  }

  std::vector<bool> ok(list_of_slaves.size(), false);
  for (int i = 1; i <= ec_slavecount; i++)
  {
    ...
    if (list_of_slaves.find(i) != list_of_slaves.end())
    {
      std::string device_name_from_rosparam = list_of_slaves.at(i);
      ...
      if (device_name_from_coe == device_name_from_rosparam) warn = false;

      ok[std::distance(list_of_slaves.begin(), list_of_slaves.find(i))] = true;
    }
    else
...
bool soem_init(const std::string &adapter_id, const double& timeout_s)
{
  printf("[%s%s%s] %sConfig the SOEM ", BOLDMAGENTA(), "START", RESET(), BOLDYELLOW());

  if (!ec_init(adapter_id.c_str()))
  {
    printf("Impossible to init the coe (adapter: '%s') ", adapter_id.c_str());
    return false;
  }
...
{
  if (!soem_init(adapter_id, timeout_s))
  {
    printf("SOEM Init failed. Abort.");
    return false;
  }

  if (!soem_check_network_configuration(list_of_slaves))
  {
...
char *soem_config(const double timeout_s,
                  bool support_dc,
                  const std::map<int, bool> &sdo_complete_access,
                  const std::map<int, PO2SOconfigFcn> &config_fcn)
{
  ...
  for (int iSlave = 1; iSlave <= ec_slavecount; iSlave++)
  {
    ...
    printf("[-----] %sSDO CA%s [ %s%s%s ]  SOEM '%s', ROSPARAM '%s'",
           BOLDBLUE(),
           RESET(),
           BOLDCYAN(),
           (std::to_string(iSlave) + "# " + ec_slave[iSlave].name).c_str(),
           RESET(),
  ...
  // DC Config ------------------------------
  printf("[-----] %sDC SUPPORT%s [ %sALL%s ] Forced to %s",
         BOLDBLUE(),
         RESET(),
         BOLDCYAN(),
         RESET(),
  ...
  {
    bool ok = ec_configdc();
    printf("[-----] %sDC SUPPORT%s [ %sALL%s ] SOEM '%s', ROSPARAM '%s'",
           BOLDBLUE(),
           RESET(),
           BOLDCYAN(),
           RESET(),
  ...
  printf("[-----] Command the state transition to SAFE_OP to all the slaves [%s%s%s]",
         BOLDMAGENTA(),
         "IN PROGRESS",
         RESET());
  ...
  else
  {
    ...
    if (ec_slave[0].state != target_state)
    {
      ...
      for (int i = 1; i <= ec_slavecount; i++)
      {
        ...
        {
          printf("Slave %d State: %s (StatusCode: %4x : %s)",
                 i,
                 coe_core::to_string(ec_state(ec_slave[i].state)).c_str(),
...
bool soem_reset_to_operational_state()
{
  ...
  for (int slave = 1; slave <= ec_slavecount; slave++)
  {
    printf("%d # %s slave is in %s state.",
           slave,
           std::string(ec_slave[slave].name).c_str(),
           ::coe_core::to_string((ec_state)ec_slave[slave].state).c_str());
    if (ec_slave[slave].state != EC_STATE_OPERATIONAL)
    {
      ...
      else if (ec_slave[slave].state > EC_STATE_NONE)
      {
        if (ec_reconfig_slave(slave, 500))
        {
          ...
          printf(
              "slave %d reconfigured, state %s", slave, coe_core::to_string((ec_state)(ec_slave[slave].state)).c_str());
        }
      }
...
std::string to_string(const ec_err_type &t)
{
  ...
  switch (t)
  {
    ...
    case EC_ERR_TYPE_EOE_INVALID_RX_DATA:
      ret = "EC_ERR_TYPE_EOE_INVALID_RX_DATA";
      break;
 ...
 *
 */
template<coe_core::PdoType PDO_TYPE>
bool get_pdo_map_through_sii(coe_core::Pdo<PDO_TYPE> &pdo,
                             const uint16_t &iSlave,
                             const uint16_t &SIIcategory,
                             std::size_t mapoffset,
                             std::size_t bitoffset)
#define NEW_DATA_OBJECT( USER_ECT_TYPE )\
  obj.reset(new coe_core::DataObjectEntry< coe_core::std_type<USER_ECT_TYPE >::type > (obj_idx, obj_subidx, std::string(str_name).c_str(), coe_core::std_type< USER_ECT_TYPE >::value) );
{
  ...
  if (PDO->Startpos > 0)
  {
    ...
    do
    {
      ...
      {
        str_name[0] = 0;
        if (obj_name) ec_siistring(str_name, iSlave, obj_name);
        ...
        for (uint16 er = 1; er <= nEntries; er++)
        {
          ...
          if (obj_idx || obj_subidx)
          {
            ...
            switch (obj_datatype)
            {
              case ECT_BOOLEAN    : NEW_DATA_OBJECT(ECT_BOOLEAN    ) break;
              case ECT_INTEGER8   : NEW_DATA_OBJECT(ECT_INTEGER8   ) break;
              case ECT_INTEGER16  : NEW_DATA_OBJECT(ECT_INTEGER16  ) break;
              case ECT_INTEGER32  : NEW_DATA_OBJECT(ECT_INTEGER32  ) break;
              case ECT_INTEGER64  : NEW_DATA_OBJECT(ECT_INTEGER64  ) break;
              case ECT_UNSIGNED8  : NEW_DATA_OBJECT(ECT_UNSIGNED8  ) break;
              case ECT_UNSIGNED16 : NEW_DATA_OBJECT(ECT_UNSIGNED16 ) break;
//              case ECT_UNSIGNED24 : NEW_DATA_OBJECT(ECT_UNSIGNED24 ) break;
              case ECT_UNSIGNED32 : NEW_DATA_OBJECT(ECT_UNSIGNED32 ) break;
              case ECT_UNSIGNED64 : NEW_DATA_OBJECT(ECT_UNSIGNED64 ) break;
              case ECT_REAL32     : NEW_DATA_OBJECT(ECT_REAL32     ) break;
              case ECT_REAL64     : NEW_DATA_OBJECT(ECT_REAL64     ) break;
              case ECT_BIT1       : NEW_DATA_OBJECT(ECT_BIT1       ) break;
              case ECT_BIT2       : NEW_DATA_OBJECT(ECT_BIT2       ) break;
              case ECT_BIT3       : NEW_DATA_OBJECT(ECT_BIT3       ) break;
              case ECT_BIT4       : NEW_DATA_OBJECT(ECT_BIT4       ) break;
              case ECT_BIT5       : NEW_DATA_OBJECT(ECT_BIT5       ) break;
              case ECT_BIT6       : NEW_DATA_OBJECT(ECT_BIT6       ) break;
              case ECT_BIT7       : NEW_DATA_OBJECT(ECT_BIT7       ) break;
              case ECT_BIT8       : NEW_DATA_OBJECT(ECT_BIT8       ) break;
              default:
          ...
          length += sprintf(buffer + length,
                            "  [0x%4.4X.%1d] 0x%4.4X:0x%2.2X 0x%2.2X",
                            abs_offset,
                            abs_bit,
                            obj_idx,
                            obj_subidx,
  ...
  pdo.finalize();
  std::size_t m = (*std::min_element(pdo.start_bytes_map_.begin(),
                                     pdo.start_bytes_map_.end(),
                                     [](std::pair<std::size_t, std::size_t> l, std::pair<std::size_t, std::size_t> r)
                                     { return l.second < r.second; }))
                      .second;
  for (auto &b : pdo.start_bytes_map_) b.second -= m;

  return true;
#undef NEW_DATA_OBJECT
}

template <coe_core::PdoType PDO_TYPE>
int get_pdo_map_through_sdo(coe_core::Pdo<PDO_TYPE> &pdo,
                             const uint16_t &iSlave,
                             std::size_t mapoffset,
                             std::size_t bitoffset,
                             std::string &what)
{
  ...
  std::size_t PDOassign = coe_core::toUint<PDO_TYPE>::value;
  ...
  {
      what += "The module '"+ (BOLDYELLOW() +std::string(ec_slave[iSlave].name) + RESET())
        +  "' has none PDO (wkc: "+std::to_string(wkc)+", rdat "+std::to_string(rdat)+")";
      return -1;
  }
  ...
  for (int pdo_assign_subindex = 1; pdo_assign_subindex <= nidx; pdo_assign_subindex++)  // read all PDO's
  {
    ...
    if (wkc <= 0)
    {
      what += "Error in reading the SDO (iSlave " + coe_core::to_string_hex(iSlave) + " addr: "  + coe_core::to_string_hex(pdo_entry_index) + ":" + coe_core::to_string_hex(pdo_assign_subindex) + "/"  + coe_core::to_string_hex(nidx) + " " + std::to_string(wkc);
      return -1;
    }
    ...
    wkc = ec_SDOread(
        iSlave, pdo_entry_index, 0x00, FALSE, &rdl, &subcnt, EC_TIMEOUTRXM);  // read number of subindexes of PDO
         ...
         pdo_entry_subindex++)  // for each subindex
    {
      ...
      if (obj_idx || obj_subidx)  // the object is a filler (0x0000:0x00), it introduces a shift in the packed buffer
      {
        ...
        {
          what += "Error in reading the object entity properties  obj_idx: "  + coe_core::to_string_hex(obj_idx) + ", obj_subidx: " + coe_core::to_string_hex(obj_subidx) + " " + std::to_string(wkc);
          return -1;
        }
        ...
        if (push_back)
        {
          ...
          switch (OElist.DataType[obj_subidx])
          {
            case ECT_BOOLEAN:
              ...
              break;
            case ECT_INTEGER8:
              obj.reset(new coe_core::DataObjectEntry<int8_t>(pdo_entry_index,
                                                              pdo_entry_subindex,
                                                              std::string(OElist.Name[obj_subidx]).c_str(),
                                                              *(int8_t *)&rdat2));
              break;
            case ECT_INTEGER16:
              obj.reset(new coe_core::DataObjectEntry<int16_t>(pdo_entry_index,
                                                               pdo_entry_subindex,
                                                               std::string(OElist.Name[obj_subidx]).c_str(),
                                                               *(int16_t *)&rdat2));
              break;
            case ECT_INTEGER32:
              obj.reset(new coe_core::DataObjectEntry<int32_t>(pdo_entry_index,
                                                               pdo_entry_subindex,
                                                               std::string(OElist.Name[obj_subidx]).c_str(),
                                                               *(int32_t *)&rdat2));
              break;
            case ECT_INTEGER64:
              obj.reset(new coe_core::DataObjectEntry<int64_t>(pdo_entry_index,
                                                               pdo_entry_subindex,
                                                               std::string(OElist.Name[obj_subidx]).c_str(),
                                                               *(int64_t *)&rdat2));
              break;
            case ECT_UNSIGNED8:
              obj.reset(new coe_core::DataObjectEntry<uint8_t>(pdo_entry_index,
                                                               pdo_entry_subindex,
                                                               std::string(OElist.Name[obj_subidx]).c_str(),
                                                               *(uint8_t *)&rdat2));
              break;
            case ECT_UNSIGNED16:
              obj.reset(new coe_core::DataObjectEntry<uint16_t>(pdo_entry_index,
                                                                pdo_entry_subindex,
                                                                std::string(OElist.Name[obj_subidx]).c_str(),
                                                                *(uint16_t *)&rdat2));
              break;
            case ECT_UNSIGNED32:
              obj.reset(new coe_core::DataObjectEntry<uint32_t>(pdo_entry_index,
                                                                pdo_entry_subindex,
                                                                std::string(OElist.Name[obj_subidx]).c_str(),
                                                                *(uint32_t *)&rdat2));
              break;
            case ECT_UNSIGNED64:
              obj.reset(new coe_core::DataObjectEntry<uint64_t>(pdo_entry_index,
                                                                pdo_entry_subindex,
                                                                std::string(OElist.Name[obj_subidx]).c_str(),
                                                                *(uint64_t *)&rdat2));
              break;
            case ECT_REAL32:
              obj.reset(new coe_core::DataObjectEntry<double>(pdo_entry_index,
                                                              pdo_entry_subindex,
                                                              std::string(OElist.Name[obj_subidx]).c_str(),
                                                              *(double *)&rdat2));
              break;
            case ECT_REAL64:
              obj.reset(new coe_core::DataObjectEntry<long double>(pdo_entry_index,
                                                                   pdo_entry_subindex,
                                                                   std::string(OElist.Name[obj_subidx]).c_str(),
                                                                   *(long double *)&rdat2));
              break;
      ...
      char buffer[1024] = {0};
      length += sprintf(buffer + length,
                        " PDO 0x%4.4X:0x%2.2X -> [0x%4.4X.%1d] 0x%4.4X:0x%2.2X 0x%2.2X",
                        pdo_entry_index,
                        pdo_entry_subindex,
                        abs_offset,
                        abs_bit,
      ...
      if ((wkc > 0) && OElist.Entries)
      {
        length += sprintf(buffer + length,
                          " %-12s %s\n",
                          coe_core::dtype2string(OElist.DataType[obj_subidx], true).c_str(),
    ...
    const uint16_t &iSlave, coe_core::RxPdo &rx_pdo, coe_core::TxPdo &tx_pdo, char *IOmap, std::string &what)
{
  ...
  wkc = ec_SDOread(iSlave, ECT_SDO_SMCOMMTYPE, 0x00, FALSE, &rdl, &nSM, EC_TIMEOUTRXM);

  if ((wkc <= 0) || (nSM <= 2)) /* positive result from slave ? */
  {
    what += "Error in accessing the SM configuration, module '" + (BOLDRED() + std::to_string(iSlave) + RESET());
    return -1;
  }
  ...
  for (int iSM = 2; iSM <= nSM; iSM++)  // iterate for every SM type defined
  {
    ...
    if (wkc <= 0)
    {
      what += " Warning in reading the register 0x" + coe_core::to_string_hex(ECT_SDO_SMCOMMTYPE) + ":" + std::to_string(iSM + 1);
      continue;
    }
    if ((iSM == 2) && (tSM == 2))  // SM2 has type 2 == mailbox out, this is a bug in the slave!
    {
      SMt_bug_add = 1;  // try to correct, this works if the types are 0 1 2 3 and should be 1 2 3 4
      what += " Warning in accessing the SM configuration 0x" + coe_core::to_string_hex(ECT_SDO_SMCOMMTYPE) + ":" + std::to_string(iSM + 1);
    }
    ...
    if (tSM == 3) // outputs
    {
      ret = get_pdo_map_through_sdo(rx_pdo, iSlave, 0, 0, what);
    }
    else if (tSM == 4) // inputs
    {
      ret = get_pdo_map_through_sdo(tx_pdo, iSlave, 0, 0, what);
    }
...
int get_pdo_map_through_sii(const uint16_t &iSlave, coe_core::RxPdo &rx_pdo, coe_core::TxPdo &tx_pdo, char *IOmap, std::string& what) 
{
  int ret = -1;
  what = what.length() ? what + "\n" : what;
  if (IOmap == NULL)
...
std::string slave2string(int cnt, bool verbose)
{
  ...
  if (ec_slave[cnt].hasdc) ret += " - DCParentport:" + std::to_string(ec_slave[cnt].parentport) + "\n";
  ret += " - Activeports: ";
