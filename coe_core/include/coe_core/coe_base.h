 ...
 */
template <typename T>
class DataObjectEntry : public BaseDataObjectEntry
{
 private:
  T val_;

 public:
   DataObjectEntry() = delete;
  DataObjectEntry(const DataObjectEntry& ) = delete;
  DataObjectEntry(DataObjectEntry&& ) = delete;
  ...
  {
    return verbose ? BaseDataObjectEntry::to_string(verbose) + "\tval: " + coe_core::to_string_hex(val_) + "\t" + name()
                   : coe_core::to_string_hex(val_);
  }
};

template <typename T>
DataObjectEntry<T> *cast(BaseDataObjectEntry *in)
{
...
template <typename T>
std::shared_ptr<DataObjectEntry<T>> cast(BaseDataObjectEntryPtr in)
{
...
class WeakDataObject
{
 public:
  ...
  iterator begin()
  {
    if (!finalized_) throw std::runtime_error("The WeakDataObject has not been finalized. Abort.");
  ...
  std::size_t index() const
  {
    if ((!finalized_) || (cob_list_.size() == 0))
      throw std::runtime_error(
          "WeakDataObject::index() const | The CobVector "
