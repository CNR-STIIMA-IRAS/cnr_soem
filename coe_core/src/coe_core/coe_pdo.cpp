...
void Pdo::update ( const uint8_t* data, bool prepended_time )
{
  ...
  {
    time_ = double(std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch()).count())/1.0e6;
  }
...
std::string Pdo::to_string( bool verbose ) const
{
  ...
  {
    std::string offset_bytes = ( start_bytes_map_.find( f->address() )  == start_bytes_map_.end() ) ? "N/A" :   std::to_string( start_bytes_map_.at( f->address() ) );
    std::string offset_bits  = ( start_bits_map_.find(  f->address() )  == start_bits_map_.end()  ) ? "N/A" :   std::to_string( start_bits_map_.at(  f->address() ) );
...
size_t Pdo::nBytes( bool packed ) const
{
  ...
  if      ( !finalized_ )      ret = 9999999;
  else if ( cob_list_.size() == 0 ) ret = 0;
  else if ( !packed )          ret = std::accumulate ( cob_list_.begin(),cob_list_.end(),0,[] ( size_t s, coe_core::BaseDataObjectEntryPtr cob )  { return s + cob->sizeBytes(); } );
  else                         ret = dim_bytes_;
