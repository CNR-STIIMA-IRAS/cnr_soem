...
class SdoManager {
 private:
  ...
  bool syncSdo(const std::string& desc,
    ...
    try {
      ...
      switch (sdotype) {
        case coe_ros_msgs::SetSdo::Request::TYPE_U8:
          dim = 1;
          break;
        case coe_ros_msgs::SetSdo::Request::TYPE_U16:
          dim = 2;
          break;
        case coe_ros_msgs::SetSdo::Request::TYPE_U32:
          dim = 4;
          break;
        case coe_ros_msgs::SetSdo::Request::TYPE_U64:
          dim = 8;
          break;
        case coe_ros_msgs::SetSdo::Request::TYPE_I8:
          dim = 1;
          break;
        case coe_ros_msgs::SetSdo::Request::TYPE_I16:
          dim = 2;
          break;
        case coe_ros_msgs::SetSdo::Request::TYPE_I32:
          dim = 4;
          break;
        case coe_ros_msgs::SetSdo::Request::TYPE_I64:
          dim = 4;
          break;
        default: {
      ...
      if (read_sdo) {
        ...
        retval = ec_SDOread(addr, index, (uint8)subindex, FALSE, &dim, buffer,
                            EC_TIMEOUTSAFE);
        ...
        do {
          retval = ec_SDOwrite(addr, index, (uint8)subindex, dim,
                               ((uint8)subindex == 0) ? TRUE : FALSE, buffer,
                               EC_TIMEOUTRXM);
          if (retval > 0)
            break;
      ...
      if (retval <= 0) {
        ROS_ERROR(
            "ec_SDOread failed. See CoE diagnostics for further information.");
      }

      if (nominal_dim != dim) {
        throw std::runtime_error(("Read only " + std::to_string(nominal_dim) +
                                  " bytes, while " + std::to_string(dim) +
 ...
 public:
  SdoManager(coe_master::NetworkDescriptorPtr network, bool force_sdo)
      : network_(network), nh_("~"), force_sdo_(force_sdo) {
    set_sdo_.reset(new ros::ServiceServer(
        nh_.advertiseService("coe/set_sdo", &SdoManager::setSdo, this)));
    get_sdo_.reset(new ros::ServiceServer(
        nh_.advertiseService("coe/get_sdo", &SdoManager::getSdo, this)));
  }

  bool setSdo(coe_ros_msgs::SetSdo::Request& req,
              coe_ros_msgs::SetSdo::Response& res) {
    std::string s1, s2;
    s1 = std::string("SET SDO");
    s2 = coe_master::to_string(req.index, req.subindex, req.sdotype, req.value,
                               req.timeout);

    res.success = syncSdo(req.desc, req.module_id, req.sdotype, req.index,
                          req.subindex, req.timeout, false, &(req.value[0]));

    s2 +=
        std::string(" [") + (res.success ? "OK" : "FAILED") + std::string("]");

    sdo_queries_.push_back(std::make_tuple(s1, s2, bool(res.success)));
    return true;
  }

  bool getSdo(coe_ros_msgs::GetSdo::Request& req,
              coe_ros_msgs::GetSdo::Response& res) {
    std::string s1, s2;
    s1 = std::string("GET SDO");
    s2 = req.module_id + ", " +
         coe_master::to_string(req.index, req.subindex, req.sdotype);

    res.success = syncSdo(req.desc, req.module_id, req.sdotype, req.index,
                          req.subindex, req.timeout, true, &(res.value[0]));

    s2 += std::string(", value: ") +
          coe_master::to_string(req.sdotype, res.value) +
          ", timeout: " + std::to_string(req.timeout) + std::string(" [") +
          (res.success ? "OK" : "FAILED") + std::string("]");
...
inline bool initNodes(ros::NodeHandle& nh,
                      const coe_master::NetworkDescriptorPtr network,
                      std::vector<coe_master::ModuleDescriptorPtr>& nodes) {
  try {
    ROS_INFO("[%s%s%s] %sInit the nodes of the network", BOLDMAGENTA(), "START",
             RESET(), BOLDYELLOW());

    cnr::param::node_t param_root;
    rosparam_utilities::extractParam(nh, network->getNamespace(), param_root);

    std::map<int, std::string> module_addresses_map =
        network->getAddressLabelsMap();
    for (auto module_address : module_addresses_map) {
      bool default_config =
          network->hasDefaultConfiguration(module_address.first);
      std::string msg =
          std::string(RESET()) + "[-----]" +
          std::string("[ " +
                      (BOLDBLUE() + std::string("Init Params") + RESET()) +
                      " ] ") +
          std::string("[ " +
                      (BOLDCYAN() + std::to_string(module_address.first) +
                       "# " + module_address.second + RESET()) +
                      " ] ") +
          std::string("[ " +
                      (BOLDYELLOW() +
                       std::string(default_config ? "DEFAULT" : " PARAMS") +
                       RESET()) +
                      " ] ");
      ...
      coe_master::ModuleDescriptorPtr module(new coe_master::ModuleDescriptor(
          nh, network->getNamespace(), module_address.second,
          module_address.first, default_config));

      if (!module->initNodeConfigurationFromParams()) {
        ROS_ERROR_STREAM(msg << " [ " << RED() << " ERROR " << RESET()
                             << " ] Basic Configurations from params failed. ");
        return false;
      }

      if (network->hasDefaultConfiguration(module_address.first)) {
        if (!module->initNodeCoeConfigurationFromSoem(
                module_address.first, module_address.second, true)) {
          ROS_ERROR_STREAM(msg << " [ " << RED() << " ERROR " << RESET()
                               << " ] Configurations from SOEM failed. ");
          return false;
        }
      } else {
        if (!module->initNodeCoeConfigurationFromParams(
                true))  // force sdo configuration from ROSPARAM
        {
    ...
    if (nodes.size() != nodes.size()) {
      ROS_ERROR_STREAM(" [ ERROR : Number of modules configured: "
                       << nodes.size()
    ...
    ROS_INFO("[%s%s%s] %sInit the nodes of the network ", BOLDGREEN(), "  OK ",
             RESET(), BOLDYELLOW());
  } catch (std::exception& e) {
    ROS_FATAL("--------------------------");
    ROS_FATAL("%s", e.what());
...
inline bool updateNodeConfiguration(coe_master::ModuleDescriptorPtr& module,
                                    char* IOmap) {
  bool ret = false;
  std::string HDR =
      std::string(std::string(RESET()) + std::string("[-----]") + "[ " +
                  (BOLDCYAN() + std::to_string(module->getAddress()) + "# " +
                   module->getIdentifier() + RESET()) +
                  (BOLDCYAN() +
                   std::string(module->isDefaultConfig() ? " - DEFAULT CFG"
                                                         : " - PARAMS CFG") +
                   RESET()) +
                  " ]");

  if (ec_slave[module->getAddress()].mbx_proto & ECT_MBXPROT_COE) {
    ROS_DEBUG_STREAM(HDR << " PDO Mapping through SDO [ " << BOLDMAGENTA()
                         << "RUNNING" << RESET() << "] ");
    ...
    ROS_DEBUG_STREAM(HDR << " PDO Mapping through SII [ " << BOLDMAGENTA()
                         << "RUNNING" << RESET() << "]");
  ...
  if (ret) {
    ROS_DEBUG_STREAM(HDR << " PDO Mapping Finalization [ " << BOLDMAGENTA()
                         << "RUNNING" << RESET() << "] ");
    module->getTxPdo().finalize();
    module->getRxPdo().finalize();
    ROS_DEBUG_STREAM(HDR << " PDO Mapping Finalization [ " << BOLDGREEN()
                         << " OK " << RESET() << "] ");
  }

  ROS_DEBUG_STREAM(HDR << " Handles Connection [ " << BOLDMAGENTA() << "RUNNING"
                       << RESET() << "] ");
  if (!module->connectHandles()) {
    ROS_DEBUG_STREAM(HDR << " Handles Connection [ " << RED() << "ERROR"
                         << RESET() << "] ");
    return false;
  }
  ROS_DEBUG_STREAM(HDR << " Handles Connection [ " << BOLDGREEN() << "OK"
                       << RESET() << "] ");
  module->setConfiguratedSdo(ret);
  return ret;
}

inline bool updateNodes(std::vector<coe_master::ModuleDescriptorPtr>& nodes,
                        char* IOmap,
                        bool update_ros_param_server) {
  try {
    ROS_INFO("[%s%s%s] %sUpdate the nodes of the network", BOLDMAGENTA(),
             "START", RESET(), BOLDYELLOW());
    ...
    ROS_INFO("[%s%s%s] %sUpdate the nodes of the network", BOLDGREEN(), "  OK ",
             RESET(), BOLDYELLOW());
  } catch (std::exception& e) {
    ROS_FATAL("--------------------------");
    ROS_FATAL("%s", e.what());
...
inline bool configDc(
    const std::vector<coe_master::ModuleDescriptorPtr>& nodes) {
  auto it = std::find_if(nodes.begin(), nodes.end(),
                         [](const coe_master::ModuleDescriptorPtr& m) {
                           return m->isDcEnabled();
                         });
  return it != nodes.end();
}

inline std::map<int, bool> configSdoCa(
    const std::vector<coe_master::ModuleDescriptorPtr>& nodes) {
...
inline std::vector<std::string> getNodeUniqueIDs(
    const std::vector<coe_master::ModuleDescriptorPtr>& nodes) {
  std::vector<std::string> ret(nodes.size());
  std::transform(
      nodes.begin(), nodes.end(), ret.begin(),
      [](coe_master::ModuleDescriptorPtr m) { return m->getIdentifier(); });
  return ret;
}

inline int checkModuleName(
    const std::string& id,
    const std::vector<coe_master::ModuleDescriptorPtr>& nodes,
    const bool verbose) {
  int ret = -1;
  ...
  auto it = std::find_if(nodes.begin(), nodes.end(),
                         [&id](const coe_master::ModuleDescriptorPtr& m) {
                           return m->getIdentifier() == id;
                         });
  ret = (it != nodes.end()) ? (*it)->getAddress() : -1;

  if ((ret == -1) && verbose) {
    ROS_WARN("[checkModuleName] Requested: '%s', while the name available are:",
             id.c_str());
